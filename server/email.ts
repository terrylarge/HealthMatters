...
export async function sendPasswordResetEmail(email: string, resetToken: string) {
  ...
  try {
    ...
    const resetLink = process.env.REPL_SLUG ? `https://${process.env.REPL_SLUG}.${process.env.REPL_OWNER}.repl.co/reset-password?token=${resetToken}` : `http://localhost:5000/reset-password?token=${resetToken}`;
